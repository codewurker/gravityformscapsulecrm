...
class GFCapsuleCRM extends GFFeedAddOn {
	...
	public function custom_fields_for_feed_mapping() {
		...
		if ( ! $this->is_gravityforms_supported( '2.5' ) ) {
			$placeholder = array(
				'label'   => __( 'Select a Field', 'gravityformsagilecrm' ),
			);
			array_unshift( $fields, $placeholder );
		}
	...
	public function create_person( $feed, $entry, $form ) {
		...
		$custom_fields   = $this->get_generic_map_fields( $feed, 'person_custom_fields' );
	...
	public function get_menu_icon() {
		return $this->is_gravityforms_supported( '2.5-beta-4' ) ? 'gform-icon--capsule' : 'dashicons-admin-generic';
	}
}
